...
def _score_bar_html(score: int, width: int = 160) -> str:
    ...
    return (
        f'<div style="background:#e5e7eb;border-radius:6px;height:18px;'
        f'width:{width}px;display:inline-block;vertical-align:middle;">'
        f'<div style="background:{bg};border-radius:6px;height:18px;'
        f'width:{fill}px;line-height:18px;color:#fff;font-size:11px;'
        f'font-weight:700;padding:0 6px;white-space:nowrap;">'
        f'{score}</div></div>'
    )


def _pill(text: str, bg: str = "#e0f2fe", fg: str = "#0369a1") -> str:
    return (
...
def _stat_card(value: str, label: str, color: str = "#1e293b") -> str:
    return (
        ...
        f'<div style="font-size:11px;opacity:0.85;margin-top:2px;">{label}</div>'
        f'</div>'
    )
...
def _bar_chart_row(label: str, count: int, max_count: int, color: str) -> str:
    ...
    return (
        ...
        f'<div style="width:100px;font-size:12px;color:#475569;">{_esc(label)}</div>'
        ...
        f'<div style="font-size:12px;font-weight:600;color:#1e293b;width:30px;">{count}</div>'
        f'</div>'
    )
...
def _render_job_row(job: dict[str, Any]) -> str:
    ...
    if len(skills or []) > 6:
        skills_html += f' <span style="color:#94a3b8;font-size:11px;">+{len(skills) - 6}</span>'
    ...
    try:
        ...
        flags = []
    flags_html = " ".join(_pill(f, bg="#fef2f2", fg="#b91c1c") for f in (flags or [])[:3])
    ...
    return (
        ...
        f'<div><a href="{url}" target="_blank" style="color:#1d4ed8;font-size:14px;'
        f'font-weight:600;text-decoration:none;">{title}</a></div>'
        f'<div style="color:#475569;font-size:13px;">{company}'
        f'<span style="color:#94a3b8;margin-left:8px;">{location}</span></div>'
        ...
        f'<td style="padding:12px 8px;vertical-align:top;text-align:center;'
        f'font-size:12px;color:#64748b;">{source}</td>'
        f'<td style="padding:12px 8px;vertical-align:top;text-align:center;'
        f'font-size:12px;color:#64748b;">{status_dot} {first_seen}</td>'
        f'</tr>'
    )
...
def generate_dashboard(conn: sqlite3.Connection, run_date: date | None = None) -> str:
    ...
    bucket_colors = {
        "90-100": "#16a34a", "70-89": "#22c55e",
        "50-69": "#ca8a04", "30-49": "#f97316", "0-29": "#dc2626",
    }
    ...
    daily_chart = "\n".join(
        _bar_chart_row(d[5:], n, max(daily.values()) or 1, "#6366f1")
...
<head>
  ...
  <style>
    * {{ box-sizing: border-box; margin: 0; padding: 0; }}
    body {{ background: #f1f5f9; font-family: -apple-system, BlinkMacSystemFont,
      'Segoe UI', Roboto, Helvetica, Arial, sans-serif; color: #1e293b; }}
    ...
    th {{ text-align: left; padding: 10px 16px; font-size: 12px;
      color: #64748b; font-weight: 600; border-bottom: 2px solid #e5e7eb; }}
...
<body>
  <div class="wrap">
    ...
    <div class="header">
      ...
      <div class="stats">
        {_stat_card(str(total_7d), "Jobs (7d)", "#fff")}
    ...
    <div class="card">
      ...
      <div style="overflow-x:auto;">
        <table>
          ...
          <tbody>
            {top_rows if top_rows else '<tr><td colspan="4" style="padding:24px;text-align:center;color:#94a3b8;">No jobs in the last 7 days. Run the pipeline first.</td></tr>'}
          </tbody>
        </table>
      </div>
    </div>
